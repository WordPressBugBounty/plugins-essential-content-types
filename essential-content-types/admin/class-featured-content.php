...
class Essential_Content_Featured_Content
{
	...
	function updated_messages($messages)
	{
		...
		$messages[self::CUSTOM_POST_TYPE] = array(
			...
			5  => isset($_GET['revision']) ? sprintf(
				...
				esc_html__('Content restored to revision from %s', 'essential-content-types'), wp_post_revision_title((int) $_GET['revision'], false)) : false, // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			6  => sprintf(
				wp_kses_post(
	...
	static function featured_content_query($atts)
	{
		...
		if (false != $atts['include_type'] || false != $atts['include_tag']) {
			$args['tax_query'] = array(); // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- necessary for filtering by content type/tag.
		}
	...
	static function featured_content_shortcode_html($atts)
	{
		...
		if ($query->have_posts()) {
			...
			do_action('ect_before_featured_content_loop', $layout[$atts['columns']]); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
...
?>
			 ...
			 */
			do_action('ect_after_featured_content_loop'); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		} else {
			/**
			 ...
			 */
			do_action('ect_no_featured_content_found'); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		}
...
if (! function_exists('essential_content_get_featured_content_thumbnail_link')) :
	...
	function essential_content_get_featured_content_thumbnail_link($post_id, $size)
	{
		if (has_post_thumbnail($post_id)) {
			 ...
			 */
			return '<a class="featured-content-featured-image" href="' . esc_url(get_permalink($post_id)) . '">' . get_the_post_thumbnail($post_id, apply_filters('featured_content_thumbnail_size', $size)) . '</a>'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- existing public API hook, renaming would break backward compatibility.
		}
	}
endif;
...
if (! function_exists('essential_content_no_featured_content_found')) :
	...
	add_action('ect_no_featured_content_found', 'essential_content_no_featured_content_found', 10); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
endif;
...
add_action('ect_before_featured_content_loop', 'essential_content_featured_content_section_open', 10, 1); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
...
add_action('ect_before_featured_content_loop', 'essential_content_featured_content_loop_start', 30); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
...
add_action('ect_after_featured_content_loop', 'essential_content_featured_content_loop_end', 10); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
...
add_action('ect_after_featured_content_loop', 'essential_content_featured_content_section_close', 20); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
