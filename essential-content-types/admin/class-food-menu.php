	...
	function add_to_dashboard()
	{
		...
		if (current_user_can('administrator')) {
			$number_menu_items_published = sprintf(
				...
				sprintf(
					// Translators: %1$d is a number of food menu item 
					_n('%1$d Food Menu Item', '%1$d Food Menu Items', intval($number_menu_items->publish), 'essential-content-types'),
					number_format_i18n($number_menu_items->publish)
				)
			);
		} else {
			$number_menu_items_published = sprintf(
				// Translators: %1$s is number of food menu item.
				'<span>%1$s</span>',
				sprintf(
					// Translators: %1$d is a number of food menu item 					
					_n('%1$d Food Menu Item', '%1$d Food Menu Items', intval($number_menu_items->publish), 'essential-content-types'),
		...
		echo '<li class="ect-food-menu-count">' . wp_kses_post($number_menu_items_published) . '</li>';
	}
...
?>
	...
	function show_menu_titles_in_menu_item_list($post)
	{
		...
		<tr class="no-items menu-label-row" data-term_id="<?php echo esc_attr($term_id) ?>">
			<td class="colspanchange" colspan="<?php echo (int) $non_order_column_count; ?>">
				<h3><?php
					echo esc_html(str_repeat(' &mdash; ', (int) $parent_count));
					...
					if ($term instanceof WP_Term) {
						...
						esc_html_e('Uncategorized', 'essential-content-types');
					}
					?></h3>
			</td>
			<td>
	...
	function add_many_new_items_page()
	{
		...
		<div class="wrap">
			...
			<p><?php esc_html_e('Use the <kbd>TAB</kbd> key on your keyboard to move between colums and the <kbd>ENTER</kbd> or <kbd>RETURN</kbd> key to save each row and move on to the next.', 'essential-content-types'); ?></p>

			<form method="post" action="" enctype="multipart/form-data">
				<p>
				<h3><?php esc_html_e('Add to section:', 'essential-content-types'); ?> <?php wp_dropdown_categories(array(
				...
				<table class="many-items-table wp-list-table widefat">
					<thead>
						<tr>
							...
							<th scope="col"><?php esc_html_e('Labels: <small>spicy, favorite, etc. <em>Separate Labels with commas</em></small>', 'essential-content-types'); ?></th>
	...
	function list_admin_labels($post_id = 0)
	{
		...
		if (!empty($labels)) {
			...
			echo wp_kses_post(join(_x(', ', 'Nova label separator', 'essential-content-types'), $out));
		} else {
