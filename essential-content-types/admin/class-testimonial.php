...
class Essential_Content_Jetpack_Testimonial
{
	...
	function setting_html()
	{
		if (current_theme_supports(self::CUSTOM_POST_TYPE)) : ?>
			<p><?php printf(esc_html__('Your theme supports Testimonials', 'essential-content-types')); ?></p>
		<?php else : ?>
	...
	function updated_messages($messages)
	{
		...
		$messages[self::CUSTOM_POST_TYPE] = array(
			...
			1  => sprintf(
				wp_kses_post(
					// Translators: %s is a URL to preview a updated post.						
					__('Testimonial updated. <a href="%s">View testimonial</a>', 'essential-content-types')
				),
			...
			6  => sprintf(
				wp_kses_post(
					...
					__('Testimonial published. <a href="%s">View testimonial</a>', 'essential-content-types')
				),
				esc_url(get_permalink($post->ID))
			),
			...
			8  => sprintf(
				wp_kses_post(
					...
					__('Testimonial submitted. <a target="_blank" href="%s">Preview testimonial</a>', 'essential-content-types')
				),
			...
			10 => sprintf(
				wp_kses_post(
					...
					__('Testimonial draft updated. <a target="_blank" href="%s">Preview testimonial</a>', 'essential-content-types')
				),
				esc_url(add_query_arg('preview', 'true', get_permalink($post->ID)))
			),
		);
...
if (! function_exists('essential_content_testimonial_section_open')) :
	...
	{
		echo '<div class="ect-testimonial-content-section ect-section ' . esc_html($layout) . '">';
...
if (! function_exists('essential_content_get_svg')) :
	...
	function essential_content_get_svg($args = array())
	{
		...
		if (empty($args)) {
			return __('Please define default parameters in the form of an array.', 'essential-content-types');
		}

		// Define an icon.
		if (false === array_key_exists('icon', $args)) {
			return __('Please define an SVG icon filename.', 'essential-content-types');
		}
