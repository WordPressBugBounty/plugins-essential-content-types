	...
	<header class="portfolio-entry-header entry-header">
		...
		if (get_the_post_thumbnail() !== '') : ?>
			<div class="post-thumbnail">
				<?php echo wp_kses_post(essential_content_get_portfolio_thumbnail_link(get_the_ID(), 'ect-jetpack-portfolio-featured')) ?>
		...
		<?php if (false != $atts['display_types'] || false != $atts['display_tags'] || false != $atts['display_author']) : ?>
			<div class="portfolio-entry-meta entry-meta">
				...
				if (false != $atts['display_types']) {
					echo wp_kses_post(Essential_Content_Jetpack_Portfolio::get_project_type(get_the_ID()));
				}


				if (false != $atts['display_tags']) {
					echo wp_kses_post(Essential_Content_Jetpack_Portfolio::get_project_tags(get_the_ID()));
				}

				if (false != $atts['display_author']) {
					echo wp_kses_post(Essential_Content_Jetpack_Portfolio::get_project_author(get_the_ID()));
				}
				?>
			</div>
		<?php endif; ?>
